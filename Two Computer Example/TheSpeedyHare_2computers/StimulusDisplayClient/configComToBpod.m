function configComToBpod(varargin)
% originally configCom.m written by Ian Nauhaus, Callaway lab
...
% make udp object
comState.serialPortHandle = udp(rip,'RemotePort',8844,'LocalPort',15724);
...
if ~strcmp(stat, 'open')
    disp('DisplayServerToBpod: trouble opening port; cannot proceed');
    comState.serialPortHandle=[];
    out=1;
    return;
else
    disp(['DisplayServerToBpod: connection established with Bpod machine on ' rip])
end

comState.serialPortHandle.bytesavailablefcn = @BpodMasterCallback;  
